...
int main(int argc, char** argv) {
  ...
  string hostname, fileRequested;
	...
	// Send request
	cout << "Sending request for : " << fileRequested.c_str() << endl << endl;
	...
	while (true) {
		...
		// lost packet
		if (badPkt(lossProb)) {
			cout << "{Lost} Packet with sequence number " << msg.seqNum << endl << endl;
			continue;
		}
		// corrupted packet 
		if (badPkt(corptPro)) {
			cout << "Received {Corrupted Packet} with sequence number " << msg.seqNum << endl << endl;
			continue;
		}
		// received in order packet
		if (msg.pktNum == reqPktNum) {
			cout << "Received {in order} packet[" << msg.pktNum << "] with sequence number " << msg.seqNum << endl << endl;
		...
		// packet out of order
			...
			cout << "Received {Out of order} packet with sequence number " << msg.seqNum << endl << endl;
      //resend ack
	...
	// Send repeated ACK's to make sure the sender receives the lastpkt ack
