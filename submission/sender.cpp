...
int main(int argc, char** argv) {
	...
	int base = 0;
	int expectedPkt = 0;
	cout << "Sending packets to window" << endl;
	for (expectedPkt; expectedPkt <= end && expectedPkt < pkts.size(); expectedPkt++) {
		cout << "Sending packet with sequence number " << pkts[expectedPkt].seqNum << endl;
		sendto(sockfd, &pkts[expectedPkt], sizeof(pkts[expectedPkt]), 0, (struct sockaddr*) &clientAddr, len);
	...
	while (1) {
		...
		if (timeout(curTime, record)) {
			...
			timeRecord.clear();
			for (int i = base; i < expectedPkt && i < pkts.size(); i++) {
		...
		// Packet Loss
		if (badPkt(lossProb)) {
			cout << "{Lost} ACK " << ack.seqNum << endl << endl;
			continue;
		}
		// Packet Corruption
		if (badPkt(corptPro)) {
		...
		if (ack.pktNum == pkts.size() - 1){
			cout << "Successfully transmitted all packets" << endl << endl;
				break;
		}
		...
		if (ack.pktNum >= base) {
			...
			cout << "Current Window base: " << base << endl;
			cout << "Current Window end: " << end << endl;

			for (expectedPkt; expectedPkt <= end && expectedPkt < pkts.size(); expectedPkt++) {
				cout << "Sending packet[" << pkts[expectedPkt].pktNum <<"]with sequence number "<< pkts[expectedPkt].seqNum << endl << endl;

				sendto(sockfd, &pkts[expectedPkt], sizeof(pkts[expectedPkt]), 0,
					(struct sockaddr*) &clientAddr, len);
